...
async function calculateTopTopics() {
  try {
    ...
    // Also save to local file for reference (written to project root)
    const outputPath = path.join(__dirname, '../../top-topics-result.json');
    fs.writeFileSync(outputPath, JSON.stringify(topTopicsData, null, 2));
